...
if (!is.null(feats_2026)) {
  ...
  own_2026_f <- file.path(script_dir, "splash_ownership", sprintf("ownership_%d.csv", TARGET_YEAR))
  sp_2026_d12 <- if (file.exists(own_2026_f)) {
    dt <- fread(own_2026_f)
    dt[, year := TARGET_YEAR]
    dt[, day_num := as.integer(sub("^day([0-9]+)_.*$", "\\1", day))]
    dt[day_num %in% 1:2]
  } else {
    splash_raw[year == TARGET_YEAR & day_num %in% 1:2]
  }
  ...
  # Winsorize WP per seed to prevent extrapolation outside the ti(wp,seed) training range
  wp_bounds <- train_data[wp > 0, .(wp_min = min(wp), wp_max = max(wp)), by = seed]
  global_wp_min <- min(train_data$wp[train_data$wp > 0], na.rm = TRUE)
  global_wp_max <- max(train_data$wp, na.rm = TRUE)
  for (i in seq_len(nrow(feats_2026))) {
    s <- feats_2026$seed[i]
    bounds <- wp_bounds[seed == s]
    lo <- if (nrow(bounds) > 0) bounds$wp_min else global_wp_min
    hi <- if (nrow(bounds) > 0) bounds$wp_max else global_wp_max
    if (feats_2026$wp[i] > hi) {
      cat(sprintf("  WP clipped: %s (seed %d) %.1f%% -> %.1f%% (seed max in training)\n",
                  feats_2026$name[i], s, 100*feats_2026$wp[i], 100*hi))
      feats_2026$wp[i] <- hi
    }
  }
